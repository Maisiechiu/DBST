    ...
    with torch.no_grad():
        try:
            ...
            frame_dir = video.replace("videos",
                                      "rawframes2_random_margin").replace(".mp4", "")
            # save this video's anchor frmaes' bbox(for whole image)
            bbox_dir = video.replace("videos",
                                     "rawframes2_random_margin_retina").replace(
                                         ".mp4", "")
            img_tall_dir = frame_dir.replace("rawframes2_random_margin",
                                             "rawframes2_random_margin_tall")
            ...
            for frame_idx in selected_anchor_frames_idx:
                ...
                _, _, _, __, y0_new, y1_new, x0_new, x1_new = crop_face(
                        frames[0], None, bboxes[0], abs_coord=True)
                for idx in range(start_frame, end_frame):
                    img = frames[idx]
                    img = img[y0_new:y1_new, x0_new:x1_new]
