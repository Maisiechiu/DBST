...
class Mask():
    ...
    def __init__(self, landmarks, face, channels=4, mask_region=None):
        logger.info(
            "Initializing %s: (face_shape: %s, channels: %s, landmarks: %s)",
            self.__class__.__name__, face.shape, channels, landmarks)
        self.landmarks = landmarks
        self.face = face
        self.channels = channels
        self.region = mask_region
    ...
    def merge_mask(self, mask):
        ...
        assert mask.shape[
            2] == 1 and mask.ndim == 3, "Input mask be 3 dimensions with 1 channel"
...
class dfl_full(Mask):  # pylint: disable=invalid-name
    ...
    def build_mask(self):
        ...
        jaw = (self.landmarks[0:17], self.landmarks[48:68],
               self.landmarks[0:1], self.landmarks[8:9], self.landmarks[16:17])
        eyes = (self.landmarks[17:27], self.landmarks[0:1],
                self.landmarks[27:28], self.landmarks[16:17],
...
class components(Mask):  # pylint: disable=invalid-name
    ...
    def build_mask(self):
        ...
        r_eye = (self.landmarks[17:22], self.landmarks[27:28],
                 self.landmarks[31:36], self.landmarks[8:9])
        l_eye = (self.landmarks[22:27], self.landmarks[27:28],
                 self.landmarks[31:36], self.landmarks[8:9])
        nose = (self.landmarks[27:31], self.landmarks[31:36])
        parts = [
            r_jaw, l_jaw, r_cheek, l_cheek, nose_ridge, r_eye, l_eye, nose
        ]
...
class extended(Mask):  # pylint: disable=invalid-name
    ...
    def build_mask(self):
        ...
        bot_l = np.array((ql_pnt, landmarks[36], landmarks[37], landmarks[38],
                          landmarks[39]))
        bot_r = np.array((landmarks[42], landmarks[43], landmarks[44],
                          landmarks[45], qr_pnt))
        ...
        r_eye = (landmarks[17:22], landmarks[27:28], landmarks[31:36],
                 landmarks[8:9])
        l_eye = (landmarks[22:27], landmarks[27:28], landmarks[31:36],
                 landmarks[8:9])
...
class faceregion_random(Mask):
    # define mask
    def dist(self, p1, p2):
        return math.sqrt((p1[0] - p2[0])**2 + (p1[1] - p2[1])**2)

    def eyes_mask_produce(self, landmarks):
        mask = np.zeros(self.face.shape[0:2] + (1, ), dtype=np.float32)
        (x1, y1) = landmarks[37]
        (x2, y2) = landmarks[44]
    ...
    def nose_mask_produce(self, landmarks):
        mask = np.zeros(self.face.shape[0:2] + (1, ), dtype=np.float32)
        (x1, y1) = landmarks[27]
        (x2, y2) = landmarks[33]
    ...
    def mouth_mask_produce(self, landmarks):
        mask = np.zeros(self.face.shape[0:2] + (1, ), dtype=np.float32)
        (x1, y1) = landmarks[54]
        (x2, y2) = landmarks[48]
        line = cv2.line(mask, (x1, y1), (x2, y2), color=(1), thickness=2)
    ...
    def build_mask(self):
        ...
        eyes_mask = self.eyes_mask_produce(landmarks)
        nose_mask = self.nose_mask_produce(landmarks)
        mouth_mask = self.mouth_mask_produce(landmarks)

        eyes_mouth = eyes_mask + mouth_mask
        eyes_nose = eyes_mask + nose_mask
        nose_mouth = nose_mask + mouth_mask
        eyes_nose_mouth = eyes_mask + nose_mask + mouth_mask
        ...
        if self.region != None:
            mask_type = mask_table[self.region]

        else:
            mask_type = random.choices(mask_table, weights, k=1)[0]
        ...
        elif mask_type == "eyes_mouth":
            mask = eyes_mouth

        elif mask_type == "nose_mouth":
            mask = nose_mouth

        elif mask_type == "eyes_nose":
            mask = eyes_nose

        elif mask_type == "eyes_nose_mouth":
            mask = eyes_nose_mouth
...
class faceregion_random2(Mask):
    ...
    def build_mask(self):
        ...
        # define mask
        l_eye = np.concatenate((landmarks[22:28], landmarks[42:47]))
        r_eye = np.concatenate((landmarks[17:22], landmarks[36:40]))
        nose = landmarks[27:36]
        mouth = landmarks[48:61]
        mask_table = [
            "eyes",
            "nose",
            "mouth",
        ...
        if mask_type == "eyes":
            cv2.fillConvexPoly(mask, cv2.convexHull(l_eye), 255.0)
            cv2.fillConvexPoly(mask, cv2.convexHull(r_eye), 255.0)

        elif mask_type == "nose":
            cv2.fillConvexPoly(mask, cv2.convexHull(nose), 255.0)

        elif mask_type == "mouth":
            cv2.fillConvexPoly(mask, cv2.convexHull(mouth), 255.0)
