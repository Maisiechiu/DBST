...
from DeepFakeMask import dfl_full, facehull, components, extended, faceregion_random, faceregion_random2
import cv2
import tqdm
...
def random_get_hull(landmark, img1, hull_type=None, mask_region=None):
    if hull_type == None:
        hull_type = random.choice([0, 1, 2, 3])
    if hull_type == 0:
        mask = dfl_full(landmarks=landmark.astype('int32'),
                        face=img1,
                        channels=3).mask
        return mask / 255
    elif hull_type == 1:
        mask = extended(landmarks=landmark.astype('int32'),
                        face=img1,
                        channels=3).mask
        return mask / 255
    elif hull_type == 2:
        mask = components(landmarks=landmark.astype('int32'),
                          face=img1,
                          channels=3).mask
        return mask / 255
    elif hull_type == 3:
        mask = facehull(landmarks=landmark.astype('int32'),
                        face=img1,
                        channels=3).mask
        return mask / 255
    elif hull_type == 4:
        mask = faceregion_random(landmarks=landmark.astype('int32'),
                                 face=img1,
                                 channels=3,
                                 mask_region=mask_region).mask
        return mask / 255
    elif hull_type == 5:
        mask = faceregion_random2(landmarks=landmark.astype('int32'),
                                  face=img1,
                                  channels=3,
                                  mask_region=mask_region).mask
        return mask / 255
...
def blendImages(src, dst, mask, featherAmount=0.2):
    ...
    for i in range(maskPts.shape[0]):
        dists[i] = cv2.pointPolygonTest(hull, (maskPts[i, 0], maskPts[i, 1]),
                                        True)

    weights = np.clip(dists / featherAmount, 0, 1)

    composedImg = np.copy(dst)
    composedImg[maskIndices[0], maskIndices[1]] = weights[:, np.newaxis] * src[
        maskIndices[0], maskIndices[1]] + (
            1 - weights[:, np.newaxis]) * dst[maskIndices[0], maskIndices[1]]

    composedMask = np.copy(dst_mask)
    composedMask[maskIndices[0],
                 maskIndices[1]] = weights[:, np.newaxis] * src_mask[
                     maskIndices[0], maskIndices[1]] + (
                         1 - weights[:, np.newaxis]) * dst_mask[maskIndices[0],
                                                                maskIndices[1]]
...
class BIOnlineGeneration():
    ...
    def __init__(self):
        ...
        self.data_list = ['000_0000.png', '001_0000.png'] * 10000
    ...
    def gen_one_datapoint(self):
        ...
        if random.randint(0, 1):
            ...
            if random.randint(0, 1):
                face_img = face_img.resize((aug_size, aug_size),
                                           Image.BILINEAR)
            else:
    ...
    def get_blended_face(self, background_face_path):
        ...
        foreground_face = sktransform.resize(foreground_face,
                                             (aug_size, aug_size),
                                             preserve_range=True).astype(
                                                 np.uint8)
        background_face = sktransform.resize(background_face,
                                             (aug_size, aug_size),
                                             preserve_range=True).astype(
                                                 np.uint8)
        ...
        foreground_face = colorTransfer(background_face, foreground_face,
                                        mask * 255)

        # blend two face
        blended_face, mask = blendImages(foreground_face, background_face,
                                         mask * 255)
        blended_face = blended_face.astype(np.uint8)

        # resize back to default resolution
        blended_face = sktransform.resize(blended_face, (317, 317),
    ...
    def search_similar_face(self, this_landmark, background_face_path):
        ...
        all_candidate_path = filter(lambda k: name_resolve(k)[0] != vid_id,
                                    all_candidate_path)
