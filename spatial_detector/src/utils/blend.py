...
def dynamic_blend(source, target, mask, hull_type=None):
    if hull_type == 'face_region':
        mask_blured = get_blend_mask_face_region(mask)
    else:
        mask_blured = get_blend_mask(mask)
    blend_list = [0.3, 0.5, 0.75, 1, 1, 1]
...
def get_blend_mask(mask):
    ...
    mask_blured = cv2.GaussianBlur(mask_blured, kernel_2,
...
def get_blend_mask_face_region(mask):
    H, W = mask.shape
    size_h = np.random.randint(192, 257)
    size_w = np.random.randint(192, 257)
    mask = cv2.resize(mask, (size_w, size_h))
    kernel_1 = random.randrange(3, 10, 2)
    kernel_1 = (kernel_1, kernel_1)
    kernel_2 = random.randrange(3, 10, 2)
    kernel_2 = (kernel_2, kernel_2)
    ...
    mask_blured = mask_blured / (mask_blured.max())
    mask_blured = cv2.resize(mask_blured, (W, H))
    cv2.imwrite("face_region_mask.png", mask_blured * 255)
    return mask_blured.reshape((mask_blured.shape + (1, )))
