...
class SBI_Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        try:
            ...
            #     'original_sequences', 'check_results')
            # mask_save_dir = os.path.dirname(image_paths[0]).replace(
            #     'original_sequences',
            #     'check_results').replace('/raw/', '/masks/')
            frame_save_dir = os.path.dirname(image_paths[0]).replace(
                'original_sequences', 'new_aug')
            mask_save_dir = os.path.dirname(image_paths[0]).replace(
                'original_sequences', 'new_aug').replace('/raw/', '/masks/')
